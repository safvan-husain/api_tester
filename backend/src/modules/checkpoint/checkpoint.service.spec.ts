...
const mockCheckpointModel = {
  new: jest.fn().mockImplementation((dto) => ({
    ...dto,
    save: jest.fn().mockResolvedValue({ _id: 'cpMockId', ...dto }),
  })),
...
describe('CheckpointService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CheckpointService,
        {
          provide: getModelToken(Checkpoint.name),
          useValue: mockCheckpointModel,
        },
        {
          provide: getModelToken(ApiRequest.name),
          useValue: mockApiRequestModel,
        },
    ...
    checkpointModel = module.get<Model<Checkpoint>>(
      getModelToken(Checkpoint.name),
    );
    apiRequestModel = module.get<Model<ApiRequest>>(
      getModelToken(ApiRequest.name),
    );
  ...
  describe('create', () => {
    ...
    it('should create a checkpoint and set request unsaved to false', async () => {
      mockApiRequestModel.findById.mockReturnValue({
        exec: jest.fn().mockResolvedValue(mockApiRequest),
      });
      ...
      mockCheckpointModel.new.mockImplementation(() => ({
        save: saveMock,
      }));
      (requestService.setUnsavedStatus as jest.Mock).mockResolvedValue({
        ...mockApiRequest,
        unsaved: false,
      });
      ...
      expect(mockApiRequestModel.findById).toHaveBeenCalledWith(
        createCheckpointDto.requestId,
      );
      expect(mockCheckpointModel.new).toHaveBeenCalledWith(
      ...
      expect(requestService.setUnsavedStatus).toHaveBeenCalledWith(
        createCheckpointDto.requestId,
        false,
      );
    ...
    it('should throw NotFoundException if request to checkpoint is not found', async () => {
      ...
      await expect(service.create(createCheckpointDto)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should log a warning if setting unsaved status fails but still return checkpoint', async () => {
      ...
      (requestService.setUnsavedStatus as jest.Mock).mockRejectedValue(
  ...
  describe('findAllForRequest', () => {
    ...
    it('should return checkpoints for a given request ID', async () => {
      ...
      mockCheckpointModel.find.mockReturnValue({
        ...
        exec: jest.fn().mockResolvedValue(expectedCheckpoints),
      });
    ...
    it('should throw NotFoundException if request not found when finding checkpoints', async () => {
      ...
      await expect(service.findAllForRequest(requestId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('rollback', () => {
    ...
    const mockCheckpoint = {
      ...
      requestId: requestId,
      data: {
        url: 'http://rolledback.com',
        method: 'POST',
        headers: { auth: 'bearer' },
        body: { key: 'value' },
      },
    };
    const expectedRolledBackRequest = {
      _id: requestId,
      ...mockCheckpoint.data,
      unsaved: true,
    };
    ...
    it('should rollback request to checkpoint data and set unsaved to true', async () => {
      mockCheckpointModel.findById.mockReturnValue({
        exec: jest.fn().mockResolvedValue(mockCheckpoint),
      });
      mockApiRequestModel.findByIdAndUpdate.mockReturnValue({
        exec: jest.fn().mockResolvedValue(expectedRolledBackRequest),
      });
    ...
    it('should throw NotFoundException if checkpoint is not found', async () => {
      ...
      await expect(service.rollback(checkpointId)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('remove', () => {
    it('should delete a checkpoint and return success', async () => {
      mockCheckpointModel.deleteOne.mockReturnValue({
        exec: jest.fn().mockResolvedValue({ deletedCount: 1 }),
      });

      const result = await service.remove('cp1');
      expect(mockCheckpointModel.deleteOne).toHaveBeenCalledWith({
        _id: 'cp1',
      });
